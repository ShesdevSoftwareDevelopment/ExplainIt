...
//FUNCTIONS
	...
	function used_num($n,$mail,$U_name,$ip_address,$course_number_)
	{
		...
		if (mysqli_num_rows($query) > 0)// THIS MAIL ALREADY HAS BEEN PAID FOR 02
		{
			//ATTRIBUTES//OUTPUT 11
			$f = "registration@explainit.online";
			$f_1="Explainit Online - קורס ".$course_number_." - שילמו כבר על המייל הזה";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if($_SESSION['count']<4)
	{
					...
					if($row['U_AP'] == 1)
					{
						...
						*/	
						...
						if(mysqli_num_rows($query) > 0)
						{
							...
							*/
							...
							if($code_3 == 'N')//NOT CONNECTED FROM OTHER DEVICE, NEEDS TO PAY
							{
								...
								//UPDATE USED NUMBER
								used_num($Num,$m,$U_name,$ip_address,$course_number_);
								
								//ATTRIBUTES//OUTPUT 
								$f = "registration@explainit.online";
								$f_1="Explainit Online - קורס ".$course_number_." - P03 - שימוש במספר אישור";
						...
						else
						{
							...
							//ATTRIBUTES//OUTPUT 
								$f = "registration@explainit.online";
								$f_1="".$course_number_." - P03 - סיסמה לא נכונה";
		...
		else
		{
			...
			$message = '<html lang="iw" dir="rtl"><body>';
			$message .= '<div style="width:100%;margin:auto;text-align:center;">';
			$message .= '<h4>CONFIRMATION CODE AND MAIL ARE NOT VALID</h4>';
			$message .= '<h4>מייל:</h4>';
			$message .= '<h4>'.$U_name.'</h4>';
			$message .= '<h4>IP:</h4>';
			$message .= '<h4>'.$ip_address.'</h4>';
	...
	else
	{
		...
		//ATTRIBUTES//OUTPUT 11
		$f = "registration@explainit.online";
		$f_1="Explainit Online - קורס ".$course_number_." - P03 - ניסיון תשלום של יותר מ-4 פעמים";
